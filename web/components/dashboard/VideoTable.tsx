...
export default function VideoTable({ videos }: VideoTableProps) {
  ...
  return (
    <Table className="overflow-hidden rounded-2xl border border-gray-200 dark:border-gray-800 shadow-lg">
      <TableHeader>
        <TableRow>
      ...
      <TableBody>
        {videos.map((video) => (
          <TableRow key={video.id} className="group hover:bg-gray-50 dark:hover:bg-[#111] transition-colors relative">
            {/* Thumbnail */}
  ...
  Ready
                ...
                <Badge variant="secondary">Processing {video.progress}%</Badge>
              ) : (
            ...
            <TableCell colSpan={5} className="absolute bottom-0 left-0 right-0">
              <div className="h-1 w-full bg-gray-200 dark:bg-gray-700 overflow-hidden rounded-b-lg">
                <div
                  className={`h-full ${video.progress === 100 ? "bg-green-500 dark:bg-green-600" : getProgressColor(video.progress)} transition-all duration-500`}
